...
def parse_recipient(message: str, explicit_to: Optional[str] = None) -> Tuple[str, Optional[str]]:
    ...
    if explicit_to:
        return message, resolve_recipient(explicit_to)

    # Parse @mention from message start
    # Matches: @name, @nclaude/branch, @some-session-id
    match = re.match(r'^@([\w/.-]+)\s+', message)
    if match:
        target = match.group(1)
        cleaned_message = message[match.end():]
        return cleaned_message, resolve_recipient(target)
